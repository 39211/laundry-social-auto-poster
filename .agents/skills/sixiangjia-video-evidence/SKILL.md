...
The validator's `generation_authorized` output field is retired as an authorization signal: it derives from a token chain that has never issued a token, so it read as a standing "false" that contradicted the candidate manifest's then-hardcoded "true" (BOARD0817-ABSORB, DEAD_VS_ALIVE). The only source of truth for paid generation authorization is `data/publishing-policy.json#paid_video_budget`, evaluated fail-closed by `src/generationAuthorization.ts` and published per day in the video candidate manifest. Do not persist validator output to `output/operations/*-preproduction-contract.json`; that file family was declared dead and archived to `output/operations/archive/preproduction-contracts-retired-2026-08-17/` on 2026-08-17.
