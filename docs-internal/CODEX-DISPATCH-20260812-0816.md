...
## 零點五、**動手前先讀踩坑簿**(這條在鐵則之上)

**`docs-internal/ERROR-BOOK.md`** — 這個專案已經踩過的每一個坑,含真因與修法。
每天開工前掃一遍與你今天任務相關的段落。**最常害人的五個**:

1. **改主題沒改 image_prompt** → 文不配圖上線(A1)
2. **cp950 讓中文比對變成漂亮的假陰性** → 抓取與比對必須同一個 Python 程序(B1)
3. **舊 day-lock 會把新內容打回去** → 刻意重做要先刪鎖(A3)
4. **加一次性排程觸發會蓋掉每日觸發** → 必須 `@($t.Triggers) + $once`(D2)
5. **`media_publish` 之後絕不重試** → 去查 posted-log,不要重發(E1)

**你也要維護它**:這五天踩到任何新坑,當天用同樣格式(症狀→真因→修法→怎麼再驗)加一條到 ERROR-BOOK.md 並 commit。
不寫下來的坑,下一個人會再踩一次。
...
## 七點五、**每日必做的優化與索引動作**(不是選配)

老闆的常設指令:**每天都要有一件優化,而且要留下記錄**。只看守不優化 = 沒做事。

### 每日索引推送(自動,但你要驗)
`Daily-Generate` 06:30 內含 `submit-indexnow --live` 與 `indexing-push`。
**你的驗證**:每天看 `output/operations/indexing-push-<date>.json`,確認
`indexnow_status: 200`、`submitted` > 0、`thin_pages` 長度 0。
(基準:8/8-8/11 連續四天 200,每天推 22-26 頁。)
若 `thin_pages` 非空 → 那些頁內容太薄,記下網址,回報老闆,**不要自己亂加字充數**。

### 每日一件優化(你要主動做)
從下面選**一件**做,做完寫進 `output/daily-optimization-log.md`(沒有就建),格式:
```
## 2026-08-1X
改了什麼:<一句話>
為什麼:<依據哪個數據/觀察>
怎麼驗:<當場跑的證據>
```
可選項目(依當天數據挑最該做的,不要每天做同一件):
- **鉤子迭代**:X 搜尋「洗鞋」「洗衣店」「開學 送洗」等,發現高互動句式 → append `data/hooks-bank.json`
- **標題/開頭句**:昨日 watch-time 最高那支的開頭結構,套用到明天的生成
- **新增一頁長尾 SEO**:挑一個真實搜尋意圖(急迫、有判斷內容可寫),照 `src/generatePublicSite.ts` 現有結構加,**測試計數要同步更新**,跑 `npm test` 全綠才部署
- **hashtag 組合**:依 IG insights 的來源分佈調整地標籤/意圖標籤比例
- **價格線索**:哪類物件被問最多卻沒寫進文案,補進 `data/prices.json` 的呈現規則
- **踩坑簿補一條**:今天踩到的坑

**一天只動一個變數**(鐵則 9)。優化與內容治療 A/B/C(8/12-14)衝突時,**優化讓路**——那三天只做不影響影片變數的優化(例如 SEO 頁、踩坑簿)。

### 海報進 FB/IG(老闆已定案:海報同時當貼文用)
海報 v3 已排進 8/18 的行事曆 slot1(`data/slot1-plan.json` = `海報宣傳-免費收送`)。
你要做的:8/17 檢查該日行事曆生成後,slot1 的圖有沒有換成
`output/print/poster-A4-shoes-sixiangjia-v3.png` 的 IG 方形裁切版
(`docs/assets/2026-08-18/slot-01.png`),沒有就手動換上並重鎖。
**若老闆在這五天內對 v3 有新意見,逐字記下來留給 Claude,不要自行重新設計。**
