// One-time GSC re-authorisation (mirrors gbp-authorize.mjs). Uses the same
// dedicated GBP OAuth client in secrets/gbp-oauth-client.json — the only
// client on this account confirmed to have Console access and the Search
// Console API already enabled (see memory: sixiangjia-google-oauth-clients-trap).
// Prints the GSC_REFRESH_TOKEN line to paste into .env; writes nothing
// automatically.
//
...
const server = createServer(async (req, res) => {
  ...
  res.end(
    `<meta charset="utf8"><body style="font:16px/1.7 system-ui;padding:40px">` +
      (code ? "GSC 授權完成,可以關掉這個分頁。" : `授權失敗:${error ?? "沒有拿到授權碼"}`) +
      `</body>`
  );
  server.close();
