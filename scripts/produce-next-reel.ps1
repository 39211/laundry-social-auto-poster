...
function Get-CorePhysics([string]$ObjectType, [string]$Act) {
    # F33: after reused the working-physics sentence, so the model kept cleaning.
    # before/middle keep object-specific process; after is settled/still only.
    if ($Act -eq "after") {
        return "The object is settled and still in the supplied after state. No cloth, brush, steam or hand touches the object. No wiping, scrubbing, wetting-then-lightening, or any other cleaning-in-progress motion occurs. Material, colour, remaining wear, fittings and contact shadow stay exactly as the still shows."
    }
    switch ($ObjectType) {
...
"@
    ...
    if (($targetVariant -eq "15s" -or $treatmentNeedsMiddle) -and -not (Test-Path $middlePng)) {
        ...
        # than the whole Reel.
        $middleHeader = @"
Read the image file at the path given below and EDIT it. Keep the same camera position, the same counter, the same background, the same light direction and the same white balance as that image. The featured item must remain the SAME physical object: same material, same colour, same fittings or laces, same wear marks. Add the craftsman's hands: an adult man's working hands, dry and clean with short unpolished nails, one thin old scar across the back of the left hand, forearms lightly tanned, sleeves of a faded indigo work shirt rolled to just below the elbow, a dark canvas apron edge visible at the frame bottom. No rings, no watch, no gloves. The hands and forearms enter from outside the frame and occupy a natural minority of it, as if reaching in toward the item on the counter -- the item, the counter and the background stay clearly visible around them, not a close-up filling the frame with skin. One hand holds one shop tool (soft cloth, soft-bristle brush, or steam tool as fits the object), the tool in contact with the worn area the narration is about, a small partially-cleaned patch already visible at that exact spot, the other hand steadying the item, and the rest of the item stays in its original condition. Hands anatomically correct: five fingers each, no fusing, no extra hand entering frame. No face, no head, no torso above the elbow. Portrait 4:5. No readable text, no logo, no watermark, no faces. Leave the edited image in your output directory and report the filename.
...
"@
    ...
    foreach ($state in $states) {
        ...
        foreach ($attempt in 1, 2) {
            # The manifest used to be cloned from a July run file under
            ...
            $corePhysics = Get-CorePhysics $objectType $state
