...
export function renderDailyIndex(eligible:{article:Article;assets:Asset[]}[],r:Registry,preview:boolean):string {
  ...
  const body=`<p class="tag">${esc(r.brand)}・洗衣筆記</p><h1>先看懂衣物的狀況，<br>再一起決定怎麼照顧。</h1><p>從照片怎麼拍、材質怎麼看，到與門市確認處理範圍。每篇回答一個具體問題，讓送洗溝通更清楚。</p><nav aria-label="筆記導覽"><a href="#notes">閱讀洗衣筆記</a> · <a href="#journey">30–90 天內容計畫</a>${preview?'':` · <a href="${esc(absolute('/',r))}">回到門市網站</a>`}</nav><section id="notes" aria-label="洗衣筆記文章"><p>${preview?'本次預覽':'目前公開'} ${eligible.length} 篇</p><div class="cards">${cards}</div></section><section id="journey" class="body" aria-labelledby="journey-title"><h2 id="journey-title">把日常的洗護問題，慢慢整理成一本筆記</h2><p>這是我們的 30–90 天內容計畫。下列是持續擴充的方向，已完成可閱讀的內容以上方文章為準。</p><h3>前 30 天｜從送洗前的疑問開始</h3><p>怎麼拍清楚衣物、如何查看材質、哪些狀況要先問門市。用完整文章與圖片，把每一次送洗的準備說明白。</p><h3>第 31–60 天｜把常見問題說得更細</h3><p>依季節、衣物種類與讀者提問補充筆記，回頭修訂需要更清楚的說明，串起相關的洗護知識。</p><h3>第 61–90 天｜讓內容更好找、更好用</h3><p>整理主題索引、補充圖解與通過審查的影片，讓新讀者能從自己的問題找到適合的內容。</p><p>文章、圖片與影片會分批製作與確認；計畫不代表所有素材已完成，也不承諾每天新增一個網頁。AI 示意圖片不代表實際洗前洗後成果。</p></section>`;
